...
async function build(c, options = {}) {
    ...
    (0, log_1.log)("THIS IS OUR SPECIAL VERSION GOOD JOB YOU GOT IT TO RUN IN OTTO YAAAY");
    if (await isPnpmProject(c.root)) {
        if (options.tarball)
            throw new Error('Tarball is not supported for pnpm projects.');
    ...
    await (0, bin_1.writeBinScripts)({ baseWorkspace: c.workspace(), config: c.config, nodeOptions: c.nodeOptions, nodeVersion: c.nodeVersion });
    await pretarball(c);
